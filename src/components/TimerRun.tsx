...
import { type SessionType, usePomodoroStats } from "../hooks/usePomodoroStats";
...
export default function TimerRun({
	...
	const { addSession, history, hours, minutes, sessionCount, weeklyStats } =
	...
	useEffect(() => {
		...
		if (isActive && timeLeft > 0) {
			...
			if (!processedSessionsRef.current.has(currentSessionIndex)) {
				...
				if (Notification.permission === "granted") {
					new Notification(
						`¡${currentSession.label} ${t("timer.run.finished")}!`,
						{
							body:
								lang === "en"
		...
		schedule,
		blockStartTime,
	]); // Removed currentSession from deps to avoid circularity issues if any
		...
		return (
			<div className="flex flex-col items-center justify-center p-10 space-y-4">
				<h2 className="text-2xl font-bold">
					{lang === "en" ? "Session Error" : "Error en la sesión"}
				</h2>
				<p>
					{lang === "en"
						? "Could not recover current session."
				...
				<div className="w-full bg-base-100/50 backdrop-blur-sm rounded-2xl p-6 border border-base-200 shadow-xl transition-colors duration-400">
					<div className="mb-6 pb-4 border-b border-base-200 flex justify-between items-center">
						<div className="flex flex-col">
							...
							<span className="text-xs font-mono opacity-60">
								{t("timer.run.start")} {formatHour(planStartTime)}
							</span>
						</div>
					...
					<div className="relative flex items-center overflow-x-auto pb-8 pt-4 custom-scrollbar scroll-smooth snap-x">
						...
						{schedule.map((session) => {
							const pos = schedule.indexOf(session);
							const isPast = pos < currentSessionIndex;
							const isCurrent = pos === currentSessionIndex;
							const sTheme = getTheme(session.type);
							return (
								<div
									key={session.label + session.type + session.duration}
									className={`relative shrink-0 flex flex-col items-center px-6 snap-center transition-all ${isCurrent ? "opacity-100 scale-105" : "opacity-50"}`}
								>
									<div
										className={`w-4 h-4 rounded-full border-2 transition-colors z-10 mb-4 ${
											isPast
												? "bg-success border-success"
												: isCurrent
													? `${sTheme.bgButton} border-white shadow-lg`
													: "bg-base-100 border-base-300"
										}`}
									>
										{isPast && (
											<svg
												className="w-2.5 h-2.5 text-white absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
												fill="none"
												viewBox="0 0 24 24"
												stroke="currentColor"
												strokeWidth="3"
									...
									<div
										className={`p-3 rounded-lg border-b-4 w-40 text-center ${isCurrent ? `bg-base-200 shadow-md ${sTheme.border}` : "border-transparent"}`}
									>
							...
							<div className="opacity-30 blur-sm pointer-events-none filter grayscale">
								...
								<div className="flex justify-between items-end h-32 gap-2">
									{[1, 2, 3, 4, 5, 6, 7].map((n) => (
										<div
											key={`ph-${n}`}
											className="bg-base-300 w-full rounded-t-lg"
							...
							<div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-base-100/10 backdrop-blur-[2px] p-6 text-center">
								<div className="bg-base-100/90 p-5 rounded-xl shadow-2xl border border-base-content/5 backdrop-blur-xl transform transition-transform hover:scale-105">
									...
									<p className="text-xs opacity-70 mb-4 max-w-[200px] leading-relaxed">
										{lang === "es"
											? "Regístrate para ver tu progreso semanal y guardar tu historial en la nube."
											: "Sign up to see your weekly progress and sync your history to the cloud."}
									</p>
									<a
										href={`/${lang === "es" ? "login" : "en/login"}`}
										className="btn btn-primary btn-sm btn-wide shadow-lg shadow-primary/20"
									>
										{lang === "es"
